...
async def handle_telegram_password(update: Update, context: ContextTypes.DEFAULT_TYPE, password: str):
    ...
    try:
        ...
        # ИСПРАВЛЕНИЕ: Загружаем актуальную сессию и сохраняем конфигурацию
        session_data = await db.load_user_session(user_id)
        logger.info(f"Текущая сессия перед сохранением конфигурации клиента: {session_data}")
...
async def process_telegram_code(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    try:
        ...
        if 'password' in error_str or 'two-factor' in error_str or '2fa' in error_str:
            logger.info(f"Пользователю {user_id} требуется двухфакторная аутентификация (обнаружено по ошибке)")
            keyboard = [[get_back_button()]]
            reply_markup = InlineKeyboardMarkup(keyboard)
